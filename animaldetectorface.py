...
face_recognizer.read('modeloLBPHFaceanimal.xml') # nombre del modelo guardado xml
...
while True:
    ...
    for (x,y,w,h) in rostro:
        ...
        if prediccion[1] < 90: #para determinar si es el animal que esta en la base
            cv2.putText(x2, '{}'.format(imgPaths[prediccion[0]]),(x,y-25),2,1.1,(0,255,0),1,cv2.LINE_AA)
            cv2.rectangle(x2, (x,y), (x+w, y+h), (0,255,0),2)
        else:
            cv2.putText(x2,'Animal Desconocido', (x, y-20), 2, 0.8, (0,0,255),1, cv2.LINE_AA)
            cv2.rectangle(x2, (x, y), (x + w, y + h), (0, 255, 0), 2)
